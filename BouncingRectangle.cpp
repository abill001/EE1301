    ...
    case WM_PAINT:
        {
			...
			Rectangle(hdc, x, y, x + LENGTH, y + WIDTH); // Draw a rectangle of length LENGTH pixels and width WIDTH pixels
			y += deltaY * diry;
			x += deltaX * dirx;
			if (y + WIDTH > visibleRegion.bottom - deltaY  || y < visibleRegion.top + deltaY) //To allow no boundary crossing
				diry *= - 1;
			if (x  + LENGTH > visibleRegion.right - deltaX || x < visibleRegion.left + deltaX) //To allow no boundary crossing
				dirx *= -1;
			Sleep(1);
